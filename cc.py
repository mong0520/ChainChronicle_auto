...
class ChainChronicleAutomation():
            ...
            if r['res'] == 0:
                try:
                    for record in r['body'][1]['data']:
                        # idx = int(self.__getCardInfoByKey(r, 'idx'))
                        # id = int(self.__getCardInfoByKey(r, 'id'))
                        # type = int(self.__getCardInfoByKey(r, 'type'))
                        idx = record['idx']
                        id = record['id']
                        type = record['type']
                        # self.logger.debug(idx)
                        ...
                        if type == 0:
                            ...
                            self.logger.debug("#{0}: 轉蛋開始！ 獲得[{1}]一張".format(i, self.cardTypes[type]))                   
    ...
    def __getQuest(self, qtype, qid):
        ...
        payload = "nature=cnt%3d{0}%26fid%3d447058%26pt%3d0%26qid%3d{2}%26type%3d{1}".format(hexNow, qtype, qid)
